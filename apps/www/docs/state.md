...
`Map`, `Set`, `Date`, and typed arrays are allowed and cloned, but they are **not** mutation-proof after freeze. Cyclic structures are supported. Functions and DOM nodes throw `TypeError: SvenJS: state must be structured-cloneable` and leave the previous state in place.
