...
A CDN / script-tag build is emitted as `svenjs.iife.js` for the original “drop it in a page” goal. Pin the version you install (`svenjs@3.4.0`). Development diagnostics are a separate file: `svenjs.iife.dev.js` or the `development` export condition on `"svenjs"`. Production imports stay on `svenjs.js`.
